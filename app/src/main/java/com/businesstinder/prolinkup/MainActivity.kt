...
import com.businesstinder.prolinkup.Cards.arrayAdapter
import com.businesstinder.prolinkup.Cards.cards
import com.businesstinder.prolinkup.Matches.MatchesActivity
...
class MainActivity : AppCompatActivity() {
    ...
    private fun isConnectionMatch(userId: String) {
        val currentUserConnectionsDb = usersDb!!.child(currentUId.toString()).child("connections").child("yeps").child(userId)
        currentUserConnectionsDb.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    usersDb!!.child(dataSnapshot.key.toString()).child("connections").child("matches").child(
                        currentUId.toString()
                    ).child("ChatId").setValue(key)
                    usersDb!!.child(currentUId.toString()).child("connections").child("matches").child(
        ...
        get() {
            usersDb!!.addChildEventListener(object : ChildEventListener {
                override fun onChildAdded(dataSnapshot: DataSnapshot, previousChildName: String?) {
                    if (dataSnapshot.child("sex").value != null) {
                        if (dataSnapshot.exists() && !dataSnapshot.child("connections").child("nope").hasChild(
                                currentUId.toString()
